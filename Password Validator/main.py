...
def passwordValidator():
   ...
   # to the rules above
    if not(6 <= len(userPassword) <= 12):
        message = (
            'Invalid Password..your password should have a minimum '
            + 'length of 6 and a maximum length of 12'
        )
    ...
    if ' ' in userPassword:
       ...
       return message
    if all(i not in string.ascii_letters for i in userPassword):
        message = (
        ...
        return message
    if all(i not in string.digits for i in userPassword):
        message = 'Invalid Password..your password should contain at least a number'
        return message
    if any(i in string.punctuation for i in userPassword):
